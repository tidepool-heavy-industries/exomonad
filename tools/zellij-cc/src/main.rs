...
fn wrap_claude(result_fifo: &Path, cwd: Option<&PathBuf>, session_tag: Option<&str>, claude_args: &[String]) -> Result<()> {
    ...
    for line in reader.lines() {
        ...
        match serde_json::from_str::<StreamEvent>(&line) {
            ...
            Err(e) => {
                // Log parse errors with enough context to debug
                let truncated: String = line.chars().take(500).collect();
                let suffix = if line.chars().count() > 500 { "..." } else { "" };
                eprintln!("[parse error] {}", e);
                eprintln!("[parse error] line ({} chars): {}{}", line.chars().count(), truncated, suffix);
                // Also store for inclusion in result
                parse_errors.push(format!("{}: {}{}", e, truncated, suffix));
            }
        }
    }
    ...
    // Build final result
    let result = build_run_result(events, result_event, status, session_tag, interrupts, parse_errors)?;
