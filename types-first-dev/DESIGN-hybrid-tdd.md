...
| hMerge | `merge-{session}` | Cherry-pick + detect conflicts | `merge: combine tests + impl` |
| hConflictResolve | `merge-{session}` | Resolve git conflicts via LLM | `conflict: resolve merge conflicts` |
...
sequenceDiagram
    ...
    alt Cherry-pick conflicts
        Merge->>Merge: hConflictResolve (LLM resolves conflicts)
        Merge->>Merge: git add && git commit
    end
...
┗━━━━━━━━━┳━━━━━━━━━━━┛      ┗━━━━━━━━━━━━━━━┳━━━━━━━━━━━━━━┛
    ...
    ┗━━━━━━━━━━━━━━━━┛           ║    │    :@ tests-worktree                 ║
                                 ...
                                 ┃                                          ┃
                                 ┃  [clean]    → Goto "hValidate"           ┃
                                 ┃  [conflict] → Goto "hConflictResolve"    ┃
                                 ┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛
                                      │                    │
                          [clean]     │                    │ [conflict]
                                      │    ┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┓
                                      │    ┃  hConflictResolve (LLMNode)   ┃
                                      │    ┃  Resolve git conflict markers ┃
                                      │    ┃  Combine both agents' changes ┃
                                      │    ┃  Verify build passes          ┃
                                      │    ┗━━━━━━━━━━━━━━┳━━━━━━━━━━━━━━━━┛
                                      │                   │
                                      └───────────────────┘
...
data TypesFirstGraphHybrid mode = TypesFirstGraphHybrid
  ...
  , hTestsReject :: mode :- G.LogicNode
    ...
    -- Routes to hConflictResolve if cherry-picks produce git conflicts.
  , hMerge :: mode :- G.LogicNode
      :@ Input VerifiedResults
      :@ UsesEffects '[ Goto "hValidate" MergedState       -- Clean merge
                      , Goto "hConflictResolve" ConflictState  -- Conflicts detected
                      ]

    -- | Resolve git conflicts via LLM agent.
    -- When tests and impl modify the same file (e.g., both add exports to .cabal),
    -- spawn an agent to resolve the conflict markers intelligently.
    --
    -- Schema: ConflictResolveOutput (files resolved, resolution notes)
    -- Handler verifies resolution compiles before proceeding.
  , hConflictResolve :: mode :- G.LLMNode
      :@ Input ConflictState
      :@ Template ConflictResolveTpl
...
-- Contains all context needed for intelligent resolution.
data ConflictState = ConflictState
  { csVerifiedResults  :: VerifiedResults    -- Original verified blind results
  , csMergeWorktree    :: FilePath           -- Where conflicts exist
  , csConflictedFiles  :: [(FilePath, Text)] -- (path, content with markers)
  , csTestsContext     :: Text               -- What tests agent was doing
  , csImplContext      :: Text               -- What impl agent was doing
  }
  deriving stock (Show, Eq, Generic)
  deriving anyclass (FromJSON, ToJSON)
...
### ConflictResolveTpl (Resolve git merge conflicts)

Template context: `ConflictState` (contains conflict markers and agent context)

```jinja
# Task: Resolve Git Merge Conflicts

The tests agent and impl agent both modified some files, creating git conflicts.
Resolve these conflicts intelligently by understanding what each agent intended.
...
{% for file in ctx.csConflictedFiles %}
### {{ file.path }}
...
1. **Resolve all conflicts** (via ClaudeCode):
   - Read each conflicted file
   - Understand what both agents intended
   - Edit to combine BOTH changes (don't drop either side)
   - For .cabal files: combine exposed-modules, dependencies, test suites
   - For Haskell: merge imports, combine exports, integrate code

2. **Verify the resolution**:
   - Run `cabal build` to ensure it compiles
   - Both tests and impl code should be present

3. **Commit the resolution**:
   - Stage resolved files with `git add`
   - Commit with message describing resolution

4. **Return structured output**:
   - List of files you resolved
   - Brief description of how each was resolved

## Common Conflict Patterns

- **exposed-modules**: Combine both lists (tests adds TestSpec, impl adds Module)
- **imports**: Include all imports from both sides
- **exports**: Combine export lists
- **build-depends**: Union of dependencies

## Output Schema

Return JSON matching `ConflictResolveOutput`:
- `croFilesResolved`: List of file paths that were resolved
- `croBuildPassed`: Did `cabal build` succeed?
- `croResolutionNotes`: Brief description of how conflicts were resolved
- `croBlocker`: If resolution failed, explain why (or null if success)
```
