...
-- | Interpreter for single-node Work graph.
--
...
module TypesFirstDev.WorkInterpreter
  ( runWork
  ) where
...
--
-- This is simpler than the old Plan/Impl interpreter because everything
-- is a self-loop. The handler does the routing logic; the interpreter
