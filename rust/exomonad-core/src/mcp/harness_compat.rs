...
pub fn coerce_harness_value(value: Value) -> Value {
    match value {
        Value::Object(obj) => Value::Object(
            obj.into_iter()
                .map(|(k, v)| (k, coerce_harness_value(v)))
                .collect(),
        ),
        Value::Array(arr) => Value::Array(arr.into_iter().map(coerce_harness_value).collect()),
        Value::String(s) => decode_if_harness_encoded(&s).unwrap_or(Value::String(s)),
        other => other,
    }
}
...
fn decode_if_harness_encoded(s: &str) -> Option<Value> {
    let trimmed = s.trim();
    if trimmed.is_empty() {
        return None;
    }
    ...
    match (shape, &decoded) {
        ...
        | (HarnessShape::Object, Value::Object(_)) => Some(coerce_harness_value(decoded)),
        _ => None,
    }
}
...
impl HarnessShape {
    fn detect(s: &str) -> Option<Self> {
        ...
        if first == '-' || first.is_ascii_digit() {
            ...
            if chars.clone().any(|c| c.is_ascii_digit())
...
mod tests {
    ...
    fn test_decode_numbers() {
        assert_eq!(decode_if_harness_encoded("841"), Some(json!(841)));
        assert_eq!(decode_if_harness_encoded("-5"), Some(json!(-5)));
        assert_eq!(decode_if_harness_encoded("1.5"), Some(json!(1.5)));
        assert_eq!(decode_if_harness_encoded("0"), Some(json!(0)));
    }

    #[test]
    fn test_decode_bools() {
        assert_eq!(decode_if_harness_encoded("true"), Some(json!(true)));
        assert_eq!(decode_if_harness_encoded("false"), Some(json!(false)));
    }

    #[test]
    fn test_decode_arrays() {
        assert_eq!(decode_if_harness_encoded("[1,2]"), Some(json!([1, 2])));
        assert_eq!(decode_if_harness_encoded("[]"), Some(json!([])));
        assert_eq!(
            decode_if_harness_encoded("[\"a\",\"b\"]"),
            Some(json!(["a", "b"]))
        );
    }
    ...
    fn test_decode_nested_coercion() {
        ...
        assert_eq!(decode_if_harness_encoded("[[\"1\"]]"), Some(json!([[1]])));
        assert_eq!(
            decode_if_harness_encoded("{\"n\":\"42\"}"),
            Some(json!({"n": 42}))
        );
    }
    ...
    fn test_decode_rejects_unshaped() {
        assert_eq!(decode_if_harness_encoded("hello"), None);
        assert_eq!(decode_if_harness_encoded("my-slug"), None);
        ...
        assert_eq!(decode_if_harness_encoded("2026-04-10"), None);
        assert_eq!(decode_if_harness_encoded(""), None);
    }

    #[test]
    fn test_decode_rejects_malformed() {
        assert_eq!(decode_if_harness_encoded("[incomplete"), None);
        assert_eq!(decode_if_harness_encoded("{broken"), None);
    }
    ...
    fn test_decode_null_passthrough() {
        assert_eq!(decode_if_harness_encoded("null"), None);
    }

    #[test]
    fn test_coerce_primitive_passthrough() {
        assert_eq!(coerce_harness_value(json!(5)), json!(5));
        assert_eq!(coerce_harness_value(json!(true)), json!(true));
        assert_eq!(coerce_harness_value(Value::Null), Value::Null);
    }

    #[test]
    fn test_coerce_plain_string_passthrough() {
        assert_eq!(coerce_harness_value(json!("hello")), json!("hello"));
        assert_eq!(
            coerce_harness_value(json!("my-agent-name")),
            json!("my-agent-name")
        );
    }
    ...
    fn test_coerce_encoded_scalar_in_string() {
        assert_eq!(coerce_harness_value(json!("841")), json!(841));
        assert_eq!(coerce_harness_value(json!("true")), json!(true));
    }
    ...
    fn test_coerce_object_with_mixed_fields() {
        ...
        assert_eq!(coerce_harness_value(input), expected);
    }
    ...
    fn test_coerce_empty_containers() {
        assert_eq!(coerce_harness_value(json!({})), json!({}));
        assert_eq!(coerce_harness_value(json!([])), json!([]));
    }

    #[test]
    fn test_coerce_null_preserved() {
        assert_eq!(coerce_harness_value(json!({"k": null})), json!({"k": null}));
        assert_eq!(coerce_harness_value(json!("null")), json!("null"));
    }
    ...
    fn test_coerce_preserves_existing_native_types() {
        ...
        assert_eq!(coerce_harness_value(input.clone()), input);
    }
    ...
    fn test_coerce_is_idempotent() {
        let input = json!({"a": {"b": {"c": "[{\"k\":\"5\"}]"}}});
        let first = coerce_harness_value(input);
        let second = coerce_harness_value(first.clone());
