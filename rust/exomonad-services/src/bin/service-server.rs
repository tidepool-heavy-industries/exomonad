...
async fn main() -> Result<()> {
    ...
    let anthropic = AnthropicService::from_env().unwrap_or_else(|e| {
        tracing::warn!(
            "Failed to initialize Anthropic service: {}. Falling back to dummy.",
            e
        );
    ...
    let github = GitHubService::from_env().unwrap_or_else(|e| {
        tracing::warn!(
            "Failed to initialize GitHub service: {}. Falling back to dummy.",
            e
        );
    ...
    let ollama = OllamaService::from_env().unwrap_or_else(|e| {
        tracing::warn!(
            "Failed to initialize Ollama service: {}. Falling back to default.",
            e
        );
    ...
    let otel = OtelService::from_env().unwrap_or_else(|e| {
        tracing::warn!(
            "Failed to initialize Otel service: {}. Falling back to dummy.",
            e
        );
        OtelService::new(
