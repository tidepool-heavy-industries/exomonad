...
| `init.rs` | `exo init [--backend codex|claude] [--session <s>] [--recreate]` — bootstrap a node-mode ROOT. Codex is the default and runs as an ordinary independent TUI (`codex`, or `codex resume <thread>` on recreate). Node-local MCP/env/model/instructions are supplied with CLI `-c` overrides, so no global/project Codex config and no shared app-server are required. The resumable rollout UUID is captured from the controlled TUI's open local session file during MCP startup and stored in `.exo/codex-root-thread.json`; MCP `_meta.threadId` is deliberately not used because it can name a surrounding hosted conversation instead. Exomonad remains a normal deferred namespaced MCP tool set; `direct_only_tool_namespaces` must not include it because that deliberately removes it from the nested code-mode surface. Claude remains available. |
...
| `config.rs` | Minimal node-mode init config read (`backend`, `tmux_session`, `model`, child-launch policy, `[codex.roles.<role>]`, and Claude `[launch_profile.<role>]`). Backend defaults to Codex. A missing config defaults silently; a present malformed config is a loud error. |
...
| `protocol.rs` | Per-role manager/worker charter consts (`ROOT`/`TL`/`DEV`/`WORKER`/`REVIEWER`). An optional on-disk `.md` override wins during prompt-tuning. Root and spawned roles use the same runtime resolver; launch logs include the effective source and protocol hash. |
...
|------|------|-------|--------------|
| `fork_wave` | `Spawner`+`Fs`+`Git` | root, tl | Fork N TL children (own worktrees). Per-child opt-in `fork_session: bool` (default false) inherits the parent's context when supported; default-false launches fresh. Per-child `model` override (tier-capped, see below). Per-child `review: Option<bool>` override — `None` (default) inherits the spawner's own `review_enabled`, `Some(b)` stamps the child's papers with `b` and it inherits onward down that child's own subtree. Per-child `file_boundary: Vec<String>` (default empty = unrestricted) is **rendered into the child's own spec** under `SCOPE (ALLOWED PATHS)` and persisted parent-side so `merge` checks it against that child's actual diff before folding. **Refuses on a dirty worktree**, naming the offending `git status --porcelain` lines — children fork from the spawner's current commit, so uncommitted state would be invisible to them; a git error fails *closed*. `preview: true` renders every child's fully-assembled spec and spawns **nothing**. The tool's `data` also carries every spawned child's byte-exact rendered spec. Before spawning, non-empty `read_first` entries are checked against tracked files at HEAD; the wave refuses atomically if any would be invisible. Reading outside a `file_boundary` warns but does not block. |
| `spawn_dev` | `Spawner`+`Fs`+`Git` | root, tl | Spawn a dev in its own worktree. Takes the same tier-capped `model`, review override, file boundary, clean-tree gate, and read-first validation as `fork_wave`. Its `data.spec` is the byte-exact rendered task prompt. |
| `spawn_worker` | `Spawner`+`Fs` | root, tl | Spawn an ephemeral worker (inline pane). Takes the model override but is not clean-gated because it deliberately shares the parent's tree. It has no file boundary because there is no child branch for `merge` to check. |
...
| `submit_branch` | `Git`+`Process`+`Spawner`+`Fs`+`Bus` | tl, dev | **Submit a complete, merge-ready branch upward.** Runs the ordered precondition checks (committed → **needs_rebase** → `.exo/checks/pre-merge/*` scripts). If reviewers are enabled (`review_enabled`, off by default), it spawns a reviewer from the submitted branch and pushes the eventual result; otherwise it delivers `[READY]` directly. `dangerously_skip_reviewer: true` is an explicit, flagged escape hatch. Optional typed receipts carry verification and deviation evidence, including a tested-commit transfer proof; full receipts are persisted for the parent-side fold. Submission does not create a checkpoint or partial state. |
...
| `send_message` | `Bus` | root, tl | Deliver to a child by name (`to: <child>`) — **tree-edges only**; inline vs worktree is transparent. Optional `reply_to` — see [Reply threading](#reply-threading). Same sender-side wake note as `notify_parent` (a just-spawned child whose harness wake path is not ready shows as unknown/not-ready — expected, the message queues and drains when ready). |
...
Roles are backend-neutral at the domain boundary. A Codex parent launches Codex descendants using
the per-role `[codex.roles.*]` model/effort settings; the Claude backend uses `ExoRole::model()`
defaults (`sonnet` leaves, `opus` spawned TL, root inheriting the operator's choice). Every spawned
node receives an explicit model binding for its selected backend.
...
| Role | agent | tools |
|------|-------|-------|
| **Root** | Selected root backend | fork_wave, spawn_dev, spawn_worker, merge, send_message, broadcast, tree, request_review, amend_boundary |
| **Tl** | Parent-selected backend | spawns, merge, notify_parent, send_message, broadcast, submit_branch, tree, request_review, amend_boundary |
| **Dev** | Parent-selected backend | notify_parent, submit_branch |
| **Worker** | Parent-selected backend or launch profile | notify_parent |
| **Reviewer** | Parent-selected backend or launch profile | verdict, notify_parent |
...
spawns a **reviewer** in its own worktree branched off the under-review code
