...
| Role | agent | tools |
|------|-------|-------|
| **Root** | Claude (inherits the launcher's default — the human's own session) | fork_wave, spawn_dev, spawn_worker, merge, send_message, broadcast, tree |
| **Tl** | Claude (Opus) | spawns, merge, notify_parent, send_message, broadcast, submit_branch, tree |
