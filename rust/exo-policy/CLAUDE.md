...
| `verdict` | `Bus` | reviewer | A reviewer's one output → a `System(SystemMessage)` to its parent: `approve` / `deny`+msg / `changes`+branch. Deliverance triggers **reviewer teardown** (best-effort kill/reclaim, handled in `exo-node`). |
...
| **Reviewer** | Gemini | verdict, notify_parent | `stop_allow` (ephemeral; its `verdict` is already its done-signal; torn down on verdict) |
...
The reviewer is torn down (best-effort) as soon as the `verdict` is processed.
...
- **`stop`** (tl) — the **local convergence gate** (`R: Git + Log + Bus + ChildLiveness`). Blocks exit while the worktree is dirty: a parent folds a child by merging its *branch* off disk, so uncommitted work is invisible to that merge. On a **clean** exit (Allow) it delivers a `System(ChildIdle)` to the parent — but **only when the subtree is idle**, asked via `ChildLiveness::any_child_busy` (NOT pane-existence: a Gemini child idles with a live `--prompt-interactive` pane, so liveness can't mean busy — idle is tracked from messages, busy at birth/poke, idle on `ChildIdle`, with pane-death as a one-way override). A TL mid-flow with a working child stays silent rather than spamming the parent every turn. **Fails OPEN** on any git error (allow, no notify); the idle gate itself is infallible and biases against false pings (a probe failure trusts the busy-bit — the `submit_branch`/`[READY]` flow is the authoritative "done").
...
- **Reviewers:** `submit_branch` spawns a reviewer and only the sidecar escalates `[READY]` on approval. Follow-ups: a two-way colleague back-channel (submitter→reviewer reply needs `send_message` on dev), and review is currently always-on (no config to disable).
