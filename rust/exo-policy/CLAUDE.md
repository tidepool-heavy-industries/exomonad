...
- **`stop`** (tl) — the **local convergence gate** (`R: Git + Log + Bus + Topology`). Blocks exit while the worktree is dirty: a parent folds a child by merging its *branch* off disk, so uncommitted work is invisible to that merge. On a **clean** exit (Allow) it delivers a `System(ChildIdle)` to the parent — but **only when the subtree is idle** (no live descendant pane via `Topology`): a TL mid-flow with active children is not idle, so it stays silent rather than spamming the parent every turn. **Fails OPEN** on any git error (allow, no notify); on a topology error it assumes *busy* and suppresses (the idle signal is supplementary — `submit_branch`/`[READY]` covers "done" — so the bias is against false pings).
