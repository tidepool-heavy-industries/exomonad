...
- **`Bus: Fs`** — `deliver(Addressee, Message)`. The append half only; the read/cursor/watch half is the sidecar's inbound loop. The last-hop delivery mechanism (tmux-paste) is the *recipient's* concern — policy never names it. Resolving a **tombstoned** child (`Reaped`/`Died`) is `BusError::Tombstoned`, not a silently-successful append: nothing reads a dead child's inbox, and its recorded pane may since have been recycled onto a different live agent.
