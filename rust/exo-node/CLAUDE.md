...
- **Convergence teardown is wired, best-effort.** `merge` (exo-policy) reclaims the folded child (`kill_pane` + `reclaim_worktree`), and a one-shot reviewer is torn down verdict-side in `handle_system`. Residual edge: teardown is best-effort — failures are reported in the merge result string, not retried, and a child whose worktree is dirty or holds a nested worktree (e.g. a still-live reviewer) can fail to reclaim and linger.
