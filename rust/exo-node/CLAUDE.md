...
- **`Domain(Box<RawValue>)`** → `handle_domain` (domain-opaque, sidecar-consumed, never shown to the LLM unless it decides to act). Deserialized to the domain system payload (transitionally `exo_caps::ReviewVerdict`; `D::System` once the engine is generic). A **review verdict** (`ReviewApproved`/`Denied`/`Changes`) from a one-shot reviewer is applied via `apply_verdict` (which **best-effort RMW-appends a round to `.exo/reviews/{safe-branch}.json`** via the new `read_reviews`/`persist_reviews` `SystemCtx` methods), then that reviewer is torn down (`kill_pane` + `reclaim_worktree`) — teardown is **verdict-only**. An undeserializable payload is logged + skipped (tolerant, like a malformed bus line).
