...
| `exomonad` | `Exomonad` — the engine-as-generic-machinery trait: the seam that replaces the fn-pointer registry. Four associated types (`Caps`/`Role`/`System`/`Spawn`) + `role_def` + `handle_system`. `SystemCtx` (the engine context a domain's `handle_system` operates through: `own_branch`/`head_sha`/`deliver_parent`/`deliver_to_self` + the **best-effort review-log persistence** methods `read_reviews`/`persist_reviews`) + `SystemOutcome` (the lifecycle action the engine still owns). The seam traits the associated types are bound by (`RoleKind`/`SpawnSpec`/`DomainSystem`) live in `exo-caps`. See [`docs/decisions/exo-trait-refactor.md`](../../docs/decisions/exo-trait-refactor.md). |
