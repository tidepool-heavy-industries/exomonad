...
fn test_agent_spawn_batch_roundtrip() {
    ...
    // We also need to stub other host functions that might be imported by the WASM module
    // even if not called, because Extism (or Wasmtime) might require all imports to be satisfied at instantiation.
    // The list of imports depends on what the WASM module was compiled with.
    // We can provide dummy implementations for them.
    
    // Helper to create dummy function
    ...
    // Note: Tool name in Haskell is "spawn_agents", arguments are { issues: [], owner: "", ... }
    let tool_args = json!({
        "issues": ["123"],
        "owner": "owner",
        "repo": "repo",
