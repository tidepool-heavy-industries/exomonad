...
fn build_test_plugin(active_fn: Function) -> Plugin {
    ...
    let all_names = vec![
        "git_get_branch", "git_get_worktree", "git_get_dirty_files", 
        "git_get_recent_commits", "git_has_unpushed_commits", 
        "git_get_remote_url", "git_get_repo_info",
        "github_list_issues", "github_get_issue", "github_create_pr", 
        "github_list_prs", "github_get_pr_for_branch", "github_get_pr_review_comments",
        "agent_spawn", "agent_spawn_batch", "agent_cleanup", 
        "agent_cleanup_batch", "agent_list",
        "fs_read_file", "fs_write_file", "file_pr", "wait_for_copilot_review"
    ];
...
fn stateful_host_fn<I, O>(
    name: &str,
    state: Arc<Mutex<Option<(I, O)>>>
) -> Function 
where
...
use exomonad_runtime::services::git::{WorktreeInfo, RepoInfo, Commit};
...
fn test_git_get_recent_commits_roundtrip() {
    run_stateful_test(
        ...
        |_| vec![
            Commit { hash: "123".to_string(), message: "msg1".to_string(), author: "me".to_string(), date: "now".to_string() },
            Commit { hash: "456".to_string(), message: "msg2".to_string(), author: "you".to_string(), date: "then".to_string() },
        ]
    );
}
...
use exomonad_runtime::services::agent_control::{SpawnAgentInput, SpawnResult, AgentType};
...
use exomonad_runtime::services::filesystem::{ReadFileInput, ReadFileOutput, WriteFileInput, WriteFileOutput};
