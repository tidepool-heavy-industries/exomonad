...
`tui_command` appends Codex-only developer instructions that spell out this discovery path and
forbid substituting native `collaboration.spawn_agent`, which bypasses Exomonad's worktree, ledger,
review, messaging, and merge lifecycle. Claude continues to receive its existing role protocol.
