...
**Do not collapse steps 2+5** into a one-shot `new_pane(cwd, launch_cmd)` — that reopens the orphan window the two-phase split closes. Delivery to every child is tmux-paste only (CC Agent Teams native delivery was retired — see `exo-node/CLAUDE.md`), so no Teams-enabling env var is set at any child's launch. `birth_finish` also stamps `kind: core.kind` into the child's papers so bootstrap reads it back and calls `Runtime::is_inline()` correctly.
