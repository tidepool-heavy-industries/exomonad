...
| `codex` | Small stock-CLI adapter: render node-scoped `codex`/`codex resume`/`codex fork` TUI commands with `-c` MCP overrides (including Codex-only routing from native `collaboration.*` to the role's discovered deferred `mcp__exomonad__*` roster), atomically read/write real thread bindings, deliver through `codex queue`, and archive with `codex archive`. |
| `protocol` | Resolve the effective role charter from an optional repository override or the baked domain charter, hash it, and log launch provenance with the built version/revision. Root and spawned nodes share this resolver. `build.rs` invalidates revision provenance on the actual Git `HEAD`, symbolic branch ref, and `packed-refs` paths resolved by `git rev-parse --git-path`, including linked-worktree layouts. |
...
`tui_command` appends Codex-only developer instructions that distinguish native collaboration from
Exomonad and direct each role to discover its actual roster instead of enumerating operations the
role may not have. Claude continues to receive its existing role protocol.
