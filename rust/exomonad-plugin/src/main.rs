...
mod protocol;
use protocol::{PluginMessage, PopupDefinition, Component, PopupState, ElementValue, VisibilityRule};
...
impl ZellijPlugin for ExoMonadPlugin {
                             ...
                             if let Some(comp) = def.components.get(self.selected_index) {
                                match comp {
                                    Component::Checkbox { id, .. } => {
                                        if let Some(ElementValue::Boolean(b)) = state.values.get_mut(id) {
                                            *b = !*b;
                                        }
                                    }
                                    Component::Choice { id, options, .. } => {
                                         if let Some(ElementValue::Choice(idx)) = state.values.get_mut(id) {
    ...
    fn render(&mut self, _rows: usize, _cols: usize) {
        println!("ExoMonad Status: [{}] {}", self.status_state, self.status_message);
