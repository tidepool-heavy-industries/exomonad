...
impl FifoGuard {
    ...
    pub fn new(path: PathBuf) -> Result<Self> {
        ...
        mkfifo(&path, Mode::S_IRUSR | Mode::S_IWUSR).map_err(|e| ExoMonadError::FifoCreate {
            path: path.clone(),
...
impl ResultFifo {
    ...
    pub fn new() -> Result<Self> {
        let path = PathBuf::from(format!("/tmp/exomonad-{}.fifo", std::process::id()));
        Ok(Self {
    ...
    pub fn read_with_timeout(&self, timeout: Duration) -> Result<RunResult> {
        ...
        let file = File::open(path).map_err(|e| ExoMonadError::FifoOpen {
        ...
        loop {
            ...
            if timeout != Duration::ZERO && elapsed >= timeout {
                return Err(ExoMonadError::FifoTimeout { elapsed });
            }
            ...
            match poll(&mut poll_fds, poll_timeout)? {
                ...
                _ => {
                    ...
                    let content =
                        std::io::read_to_string(&file).map_err(|e| ExoMonadError::FifoRead {
                            path: path.to_path_buf(),
                            source: e,
                        })?;
...
impl SignalFifo {
    ...
    pub fn new() -> Result<Self> {
        let path = PathBuf::from(format!("/tmp/exomonad-{}.signal", std::process::id()));
    ...
    /// Get the path to the FIFO (for setting EXOMONAD_SIGNAL_FIFO env var).
    pub fn path(&self) -> &Path {
        self.guard.path()
    }
...
///
/// Used by the `exomonad signal` subcommand to send signals from
/// Claude Code to the wrap process.
pub fn write_signal(fifo_path: &Path, signal: &InterruptSignal) -> Result<()> {
    let json = serde_json::to_string(signal).map_err(ExoMonadError::JsonSerialize)?;

    let mut file = std::fs::OpenOptions::new()
        .write(true)
        .open(fifo_path)
        .map_err(|e| ExoMonadError::FifoOpen {
...
pub fn write_result(fifo_path: &Path, result: &RunResult) -> Result<()> {
    let json = serde_json::to_string(result).map_err(ExoMonadError::JsonSerialize)?;
