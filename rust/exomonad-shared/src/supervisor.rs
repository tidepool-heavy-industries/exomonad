...
//! for exomonad's use case (single subprocess per wrap invocation), but
...
impl Supervisor {
    ...
    pub fn spawn(mut cmd: Command, timeout: Option<Duration>) -> Result<Self> {
        let child = cmd.spawn().map_err(ExoMonadError::Spawn)?;
        ...
        loop {
            ...
            if let Some(timeout) = self.timeout {
                ...
                if elapsed >= timeout {
                    ...
                    self.kill()?;
                    return Err(ExoMonadError::ProcessTimeout { elapsed });
                }
            }
    ...
    pub fn try_wait(&mut self) -> Result<Option<ExitStatus>> {
        self.child.try_wait().map_err(ExoMonadError::Io)
    }
}
...
pub fn build_claude_command(
    ...
    cmd.args(claude_args)
        ...
        .env("EXOMONAD_SIGNAL_FIFO", signal_fifo_path);
