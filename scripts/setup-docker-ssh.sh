...
# This copies your local SSH key to the tidepool-ssh-keys volume on the Docker host
#
...
docker volume create tidepool-ssh-keys 2>/dev/null || true

# Run a helper container to populate the volume (need openssh for ssh-keyscan)
echo "📦 Creating helper container..."
docker run -d --name tidepool-ssh-setup \
    -v tidepool-ssh-keys:/ssh \
...
docker cp "$SSH_KEY" tidepool-ssh-setup:/ssh/id_ed25519

# Add GitHub to known_hosts
echo "🌐 Adding GitHub to known_hosts..."
docker exec tidepool-ssh-setup sh -c "ssh-keyscan -t ed25519 github.com > /ssh/known_hosts 2>/dev/null"

# Fix permissions
echo "🔒 Setting permissions..."
docker exec tidepool-ssh-setup sh -c "chmod 700 /ssh && chmod 600 /ssh/id_ed25519 && chmod 644 /ssh/known_hosts"

# Cleanup
echo "🧹 Cleaning up..."
docker rm -f tidepool-ssh-setup
