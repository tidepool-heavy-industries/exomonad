# worktree-executor - Git Worktree Effect Executor

Executes `Worktree` effects by shelling out to `git worktree` commands.
...
- **ExecutorSpec**: 9 integration tests using temp git repos

Run tests:
```bash
cabal test tidepool-worktree-executor-test
```
