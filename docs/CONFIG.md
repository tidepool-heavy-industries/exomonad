...
*   **Location:** Project root (`$PROJECT_ROOT/.mcp.json`)
*   **Purpose:** Configures the Model Context Protocol (MCP) server for Claude Code.
*   **Managed By:** Often auto-generated by `scripts/setup-exomonad.sh` or `spawn_agents`.
*   **Content:**
    ```json
    {
      "mcpServers": {
...
*   `wasm/`: Registry for compiled WASM guest plugins.
    *   `wasm-guest-dev.wasm`
    *   `wasm-guest-tl.wasm`
*   `logs/`: Log files.
