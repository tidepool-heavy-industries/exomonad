...
*   **Setup:**
    ExoMonad is auto-configured by `exomonad init`. It uses the `stdio` transport to communicate with your agent.
    ```bash
    # For manual setup or troubleshooting:
    claude mcp add exomonad -- exomonad mcp-stdio
    gemini mcp add exomonad -- exomonad mcp-stdio
    ```
*   Spawned agents get MCP config auto-generated by `write_agent_mcp_config()` using the `stdio` transport.
