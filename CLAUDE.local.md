...
All spawn tools production-ready: `spawn_subtree` (Claude worktree+tab), `spawn_leaf_subtree` (Gemini worktree+tab), `spawn_workers` (Gemini panes, ephemeral). Per-agent identity resolved via URL path: `/agents/{role}/{name}/tools` and `/agents/{role}/{name}/tools/call`.
