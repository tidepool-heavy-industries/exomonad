...
import ExoMonad.Control.RoleConfig (Role (..), roleFromText)
...
import ExoMonad.Effects.Zellij (TabId (..), Zellij, checkZellijEnv, goToTab)
...
readJsonl path =
  try @SomeException (BL.readFile path) >>= \case
    Left _ -> pure Nothing
    Right content -> do
...
enrichEvent role sessionId cwd hookName (Object o) =
  Object $
    o
      <> KeyMap.fromList
