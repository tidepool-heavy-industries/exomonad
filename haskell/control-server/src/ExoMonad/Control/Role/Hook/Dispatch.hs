...
instance (KnownSymbol name, DispatchNode name field es) 
      => GDispatchHook (M1 S ('MetaSel ('Just name) su ss ds) (K1 i field)) es where
...
instance (KnownSymbol name, FromJSON input, ToJSON output) 
      => DispatchNode name (HookHandler es input output) es where
...
instance {-# OVERLAPPABLE #-} (Generic (r (AsHandler es)), GDispatchHook (Rep (r (AsHandler es))) es) 
