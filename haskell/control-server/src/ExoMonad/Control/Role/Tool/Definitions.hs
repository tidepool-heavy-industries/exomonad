...
import ExoMonad.Graph.Types (Description, Tool, type (:@))
...
data OrchestrationTools mode = OrchestrationTools
  { spawnAgents ::
      mode
        :- Tool SpawnAgentsArgs SpawnAgentsResult
    ...
    exoStatus ::
      mode
        :- Tool ExoStatusArgs ExoStatusResult
...
data GitHubTools mode = GitHubTools
  { ghIssueList ::
      mode
        :- Tool GHIssueListArgs GHIssueListResult
        :@ Description "List GitHub issues. Filter by status (open/closed) and labels.",
    ghIssueShow ::
      mode
        :- Tool GHIssueShowArgs GHIssueShowResult
...
data KaizenTools mode = KaizenTools
  { kaizenReport ::
      mode
        :- Tool KaizenReportArgs KaizenReportResult
...
data PMEpicTools mode = PMEpicTools
  { pmEpicCreate ::
      mode
        :- Tool PMEpicCreateArgs PMEpicCreateResult
        :@ Description "Create a high-level Epic issue to track a large body of work.",
    pmEpicList ::
      mode
        :- Tool PMEpicListArgs PMEpicListResult
        :@ Description "List existing Epics.",
    pmEpicUpdate ::
      mode
        :- Tool PMEpicUpdateArgs PMEpicUpdateResult
...
data TLSpecificTools mode = TLSpecificTools
  { tlCreateIssue ::
      mode
        :- Tool TLCreateIssueArgs TLCreateIssueResult
...
data PMSpecificTools mode = PMSpecificTools
  { pmStatus ::
      mode
        :- Tool PmStatusArgs PmStatusResult
