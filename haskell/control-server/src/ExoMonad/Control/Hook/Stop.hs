...
import ExoMonad.Effects.Git (Git, WorktreeInfo (..), getCommitsAhead, getDirtyFiles, getWorktreeInfo)
import ExoMonad.Effects.GitHub (Author (..), GitHub, Issue (..), IssueState (..), PRFilter (..), PullRequest (..), Repo (..), Review (..), ReviewComment (..), closeIssue, defaultPRFilter, getIssue, getPullRequest, getPullRequestReviews, listPullRequests)
import ExoMonad.Effects.Justfile (JustResult (..), Justfile, runRecipe)
...
stopHookLogic ::
  ...
  -- | GitHub repo name (e.g., "owner/repo")
  Text ->
  -- | Whether to run pre-commit checks (from EXOMONAD_STOP_PRECOMMIT)
  Bool ->
