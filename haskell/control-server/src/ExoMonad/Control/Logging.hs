...
--
-- Format: @projectDir/.exomonad/logs/control-server-YYYY-MM-DDTHH-MM-SS.log@
...
sessionLogPath projectDir = do
  ...
  pure $ projectDir </> ".exomonad" </> "logs" </> ("control-server-" <> timestamp <> ".log")
...
withDualLogger projectDir action = do
  let logsDir = projectDir </> ".exomonad" </> "logs"
