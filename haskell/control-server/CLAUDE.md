...
MCP is accessed via HTTP transport (TCP port 7432). Role-based endpoints filter available tools.
...
2. Claude Code calls MCP
   ┌──────────────────────────────────────────────┐
   │ HTTP MCP transport (TCP port 7432)           │
...
**Endpoints:**
- `/mcp/tools`, `/mcp/call` - All tools (no filtering)
- `/role/tl/mcp/tools`, `/role/tl/mcp/call` - TL tools (full access)
- `/role/dev/mcp/tools`, `/role/dev/mcp/call` - Developer tools
- `/role/pm/mcp/tools`, `/role/pm/mcp/call` - PM tools

**How it works:**

1. Claude Code connects to a role-specific endpoint (e.g., `http://localhost:7432/role/tl/mcp`)
2. Control server filters tools based on role in `Server.hs:handleRoleMcpTools`
