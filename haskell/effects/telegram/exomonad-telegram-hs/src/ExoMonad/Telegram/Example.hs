...
--      └────────────────────────────────────┘
-- @
module ExoMonad.Telegram.Example
  ( -- * Graph Definition
    EchoGraph(..)
  , echoHandlers
...
import ExoMonad.Graph.Types (Input, UsesEffects, Exit, type (:@))
import ExoMonad.Graph.Generic (AsHandler, type (:-))
import qualified ExoMonad.Graph.Generic as G
import ExoMonad.Graph.Goto (Goto, To, GotoChoice, gotoChoice, gotoExit)
import ExoMonad.Telegram.Effect (Telegram, tgSend, tgReceive)
import ExoMonad.Telegram.Types (OutgoingMessage(..), IncomingMessage(..))
