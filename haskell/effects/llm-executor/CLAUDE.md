# LLM Executor - Anthropic/OpenAI HTTP Client
...
- Understanding how graph LLM nodes execute
...
```haskell
import Tidepool.LLM.Executor (runLLMComplete, mkLLMEnv, LLMConfig(..))
...
The executor maps HTTP errors to these typed errors.

## Key Modules

| Module | Purpose |
|--------|---------|
| `Executor.hs` | Effect interpreter, request building |
...
The executor can be mocked for testing:
...
- [session-executor/CLAUDE.md](../session-executor/CLAUDE.md) - Claude Code subprocess (alternative)
