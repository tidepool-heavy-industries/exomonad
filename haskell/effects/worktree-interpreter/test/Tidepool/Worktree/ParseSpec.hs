...
module Tidepool.Worktree.ParseSpec (spec) where

import Data.Text (Text)
import Test.Hspec

import Tidepool.Effects.Worktree (WorktreePath(..))
import Tidepool.Worktree.Interpreter (parseWorktreeList)
...
spec = describe "parseWorktreeList" $ do
  ...
  it "parses a single worktree entry" $ do
    ...
    parseWorktreeList output `shouldBe`
      [ (WorktreePath "/repo/main", "main")
      ]
  ...
  it "handles detached HEAD (no branch line)" $ do
    ...
    parseWorktreeList output `shouldBe`
      [ (WorktreePath "/repo/detached", "(detached)")
      ]
  ...
  it "strips refs/heads/ prefix from branch names" $ do
    ...
    parseWorktreeList output `shouldBe`
      [ (WorktreePath "/repo/test", "feature/nested/branch-name")
      ]
  ...
  it "handles paths with spaces" $ do
    ...
    parseWorktreeList output `shouldBe`
      [ (WorktreePath "/repo/path with spaces", "main")
      ]
  ...
  it "handles bare worktree marker" $ do
    ...
    parseWorktreeList output `shouldBe`
      [ (WorktreePath "/repo/bare.git", "(detached)")
      ]
  ...
  it "handles prunable worktree marker" $ do
    ...
    parseWorktreeList output `shouldBe`
      [ (WorktreePath "/repo/prunable", "old-branch")
      ]
