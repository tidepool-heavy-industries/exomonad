...
-- Use 'ExoMonad.Effect.Types' in exomonad-core for pure effect definitions.
module ExoMonad.Effect.Runners
  ( -- * Effect Stack
    RunnerEffects
    -- * LLM Runners
  , runLLM
...
import qualified ExoMonad.Anthropic.Client as Client
import ExoMonad.Anthropic.Client
  ( SingleCallRequest(..), SingleCallResponse(..)
  , StopReason(..), ToolUse(..)
  )
import ExoMonad.Anthropic.Types (toolUseToResultId)
import qualified ExoMonad.Storage as Storage
...
-- ToolDispatcher is re-exported from ExoMonad.Effect.Types
...
-- | Run the DecisionLog effect by appending to .exomonad/decision_log.jsonl
...
runDecisionLog = interpret $ \case
  RecordDecision trace -> sendM $ do
    createDirectoryIfMissing True ".exomonad"
    withFile ".exomonad/decision_log.jsonl" AppendMode $ \h -> do
