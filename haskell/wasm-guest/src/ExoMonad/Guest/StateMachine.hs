...
-- | Sanitize a branch name for use in KV keys: map the dot-separator to a double-hyphen, then
-- replace any other KV-illegal character (e.g. @\/@, @\@@, @+@ — anything outside
-- @[A-Za-z0-9_-]@) with a single @-@. Doing @.@ → @--@ (not @-@) keeps the separator distinct so
-- a child branch like @dev.feature@ can't collide with a literal @dev-feature@.
sanitizeBranch :: Text -> Text
sanitizeBranch = T.map (\c -> if isKvChar c then c else '-') . T.replace "." "--"

-- | Characters allowed verbatim in a KV key segment (ASCII alphanumerics, @-@, @_@).
isKvChar :: Char -> Bool
isKvChar c = (isAscii c && isAlphaNum c) || c == '-' || c == '_'
