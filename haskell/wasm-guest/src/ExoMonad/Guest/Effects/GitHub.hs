...
import ExoMonad.Guest.HostCall (callHost, host_github_list_prs, host_github_create_pr)
...
instance ToJSON PRFilter where
  toJSON (PRFilter s l) = object
    [ "state" .= s
    , "limit" .= l
    ]
instance FromJSON PRFilter where
  parseJSON = withObject "PRFilter" $ \v -> PRFilter
    <$> v .:? "state"
    <*> v .:? "limit"
...
instance ToJSON CreatePRSpec where
  toJSON (CreatePRSpec t b h base) = object
    [ "title" .= t
    , "body" .= b
    , "head" .= h
    , "base" .= base
...
instance FromJSON CreatePRSpec where
  parseJSON = withObject "CreatePRSpec" $ \v -> CreatePRSpec
    <$> v .: "title"
    <*> v .: "body"
    <*> v .: "head"
    <*> v .: "base"
