...
data AgentControl a where
  SpawnSubtreeC :: Text -> Text -> Text -> Bool -> Maybe Text -> Maybe AgentType -> PermissionFlags -> AgentControl (Either Text SpawnResult)
...
spawnSubtree :: (Member AgentControl r) => Text -> Text -> Text -> Bool -> Maybe Text -> Maybe AgentType -> PermissionFlags -> Eff r (Either Text SpawnResult)
spawnSubtree task branchName parentSessionId forkSession role agentType perms = send (SpawnSubtreeC task branchName parentSessionId forkSession role agentType perms)
...
runAgentControlSuspend = interpret $ \case
  SpawnSubtreeC task branchName parentSessionId forkSession role agentType perms -> do
    let req =
