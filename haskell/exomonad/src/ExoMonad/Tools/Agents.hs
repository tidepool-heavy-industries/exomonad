...
import ExoMonad.Effects.AgentControl qualified as AC
...
data AgentTools mode = AgentTools
  { spawn :: mode :- SpawnAgents,
    cleanup :: mode :- CleanupAgents,
    cleanupMerged :: mode :- CleanupMergedAgents,
    list :: mode :- ListAgents
  }
  deriving (Generic)
...
agentTools =
  AgentTools
    { spawn = mkHandler @SpawnAgents,
      cleanup = mkHandler @CleanupAgents,
      cleanupMerged = mkHandler @CleanupMergedAgents,
      list = mkHandler @ListAgents
    }
