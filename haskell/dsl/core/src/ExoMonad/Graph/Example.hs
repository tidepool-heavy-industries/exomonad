...
import ExoMonad.Graph.Generic qualified as G (EntryNode, ExitNode, LLMNode, LogicNode, ValidGraphRecord)
import ExoMonad.Graph.Goto (Goto, LLMHandler (..), gotoChoice, gotoExit)
import ExoMonad.Graph.Reify (ReifyRecordGraph (..), makeGraphInfo)
import ExoMonad.Graph.Template (TemplateDef (..), TypedTemplate, typedTemplateFile)
import ExoMonad.Graph.Tool (ToolDef (..))
import ExoMonad.Graph.Types (Exit, Input, LLMKind (..), Schema, Template, UsesEffects, type (:@))
import ExoMonad.Schema (HasJSONSchema (..), SchemaType (..), arraySchema, describeField, emptySchema, enumSchema, objectSchema)
...
data SessionState = SessionState
  { -- | How many searches have been performed
    totalSearches :: Int,
...
--
-- @
-- import ExoMonad.Graph.Mermaid (toMermaid)
--
-- diagram :: Text
