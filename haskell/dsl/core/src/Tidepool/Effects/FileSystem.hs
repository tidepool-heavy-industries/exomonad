...
--
-- Effect type only - interpreter lives in tidepool-filesystem-interpreter.
...
-- setupHandler = do
--   result <- createDirectory "/path/to/.tidepool"
--   case result of
--     Right () -> writeFile "/path/to/.tidepool/config" "..."
...
module Tidepool.Effects.FileSystem
  ( -- * Effect
    FileSystem(..)
