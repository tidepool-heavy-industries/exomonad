-- | Native exomonad server - Servant + WebSocket + static files.
--
...
--
-- @
-- import ExoMonad.Server (runServer, ServerConfig(..), ServerMode(..))
--
-- main :: IO ()
...
import ExoMonad.UI.Interpreter (UIContext, newUIContext, UICallback)
...
import ExoMonad.Server.Session
  ( SessionMap
  , Session(..)
  , SessionInfo(..)
  , newSessionMap
...
import ExoMonad.Server.API
  ( ExoMonadAPI
  , HealthStatus(..)
  , api
  )
import ExoMonad.Graph.Export (GraphExport)
import ExoMonad.Server.SimpleAgent (simpleAgent)
...
import ExoMonad.Effect.Log (Log, LogContext)
...
-- | REST API handlers.
restHandlers :: Maybe GraphExport -> SessionMap -> Server ExoMonadAPI
