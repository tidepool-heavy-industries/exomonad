...
heuristicScoreEdge ScoreEdgeInput {..} =
  let -- Relevance Constants
      relevanceHigh = 5
      -- \^ Perfect match (e.g. "breaks" query -> Reference edge)
      relevanceMed = 3
      -- \^ Default/Contextual relevance

      -- Risk Constants
      riskExtreme = 5
      -- \^ Type-level metaprogramming (fragile)
      riskHigh = 4
      -- \^ Branching logic/Pattern matching (semantic change risk)
      riskMed = 3
      -- \^ Data definitions (struct layout)
      riskLow = 2
      -- \^ Pure functions/Standard definitions
