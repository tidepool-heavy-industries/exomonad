...
Transform DocGen from a hand-rolled effect-based implementation into a proper graph DSL agent. This becomes the **canonical example** of how to build agents using the exomonad graph DSL, demonstrating:

- Graph-based LLM orchestration
- Teaching/training data capture via exomonad-teaching
...
| [04-integrate-teaching](04-integrate-teaching.md) | Wire exomonad-teaching for JSONL capture | 03 |
...
- `control-server/src/ExoMonad/Control/Scout/DocGen.hs` - Current BFS implementation
- `control-server/src/ExoMonad/Control/Scout/DocGen/Gemma.hs` - ScoutGemma prototype
- `control-server/src/ExoMonad/Control/Handler/MCP.hs` - MCP tool routing
- `dsl/teaching/src/ExoMonad/Teaching/LLM.hs` - Teaching interpreter

### To Create
- `control-server/src/ExoMonad/Control/Scout/Graph.hs` - DocGen graph definition
- `control-server/src/ExoMonad/Control/Scout/Graph/Types.hs` - Graph-specific types
- `control-server/src/ExoMonad/Control/Scout/Graph/Templates.hs` - LLM templates
- `control-server/src/ExoMonad/Control/Scout/Graph/Handlers.hs` - Node handlers

## Architecture Notes

### Why Graph DSL?

1. **Teaching capture** - Graph execution goes through `LLM` effect, which exomonad-teaching intercepts
