...
interface UnifiedWasmExports extends WasmExports {
  ...
  getGraphState: (graphId: string) => string | Promise<string>;
}
...
export async function loadMachine(options: LoaderOptions): Promise<GraphMachine> {
  ...
  const exports = instance.exports as unknown as UnifiedWasmExports;
  ...
  return {
    ...
    async getGraphInfo(): Promise<GraphInfo> {
      ...
      return typeof str === "string"
        ? JSON.parse(str)
        : { id: graphId, name: "Unknown", nodes: [], edges: [] } as GraphInfo;
    },
    ...
    async getGraphState(): Promise<GraphState> {
      ...
      return typeof str === "string"
        ? JSON.parse(str)
        : { phase: { type: "idle" } as const, completedNodes: [] };
    },
  };
}
